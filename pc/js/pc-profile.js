...
async function pageInit() {
  ...
  const avatarEl = document.getElementById('pcProfileAvatar');
  if (avatarEl) avatarEl.textContent = avatarChar;

  const nameEl = document.getElementById('profileName');
  if (nameEl) nameEl.textContent = user.nickname || user.username || '';

  const subEl = document.getElementById('pcProfileSub');
  if (subEl) subEl.textContent = user.email || '个人账户';
  ...
  if (state.isAdmin) {
    const btn = document.getElementById('noticeAdminBtn');
...
function renderBookDropdown() {
  ...
  dropdown.innerHTML = state.books.map(b => {
    ...
    `;
  }).join('') + `<div class="book-add-btn" id="addBookBtn">+ 新建账本</div>`;

  dropdown.querySelectorAll('.book-option').forEach(opt => {
    opt.addEventListener('click', async (e) => {
      e.stopPropagation();
      const bookId = opt.dataset.id;
...
function switchFeedbackTab(tab) {
  ...
  if (tab === 'history') loadFeedbackHistory();
}
...
window.deleteNoticeAdmin = async function(id) {
  if (!await showConfirm({ title: '删除公告', message: '确定删除该公告吗？', okText: '删除', variant: 'danger' })) return;
  try {
