...
function _loanDueInfo(l) {
  if (l.is_settled || !l.due_date) return null;
  return getDueStatus(l.due_date);
}

function _loanSortKey(l) {
  // 未结清有到期日的优先，按到期日升序；其次未结清无到期日；已结清最后
  if (l.is_settled) return 2;
  if (!l.due_date) return 1;
  return 0;
}

function renderLoans() {
  let filtered = currentLoanType === 'all' ? loansCache : loansCache.filter(l => l.type === currentLoanType);
  // 按紧急程度排序：逾期/即将到期 → 未结清无到期日 → 已结清
  filtered = filtered.slice().sort((a, b) => {
    const ka = _loanSortKey(a), kb = _loanSortKey(b);
  ...
  if (filtered.length === 0) {
    container.innerHTML = '<div class="empty-state" style="grid-column:1/-1;"><div class="emoji">💰</div><p>暂无借贷记录</p></div>';
    return;
  }
...
window.openLoanDetail = function(loanId) {
  ...
  document.getElementById('loanDetailBody').innerHTML = `
    ...
    <div class="loan-detail-meta">
      ...
      ${loan.due_date ? `<div class="loan-detail-meta-item ${!loan.is_settled ? 'meta-due-' + (getDueStatus(loan.due_date)?.status || '') : ''}">
      ...
      ${loan.note ? `<div class="loan-detail-meta-item full">
        <span class="meta-label">备注</span>
        <span class="meta-value">${esc(loan.note)}</span>
      </div>` : ''}
      ${loan.image_url ? `<div class="loan-detail-meta-item full">
    ...
    <div class="repay-timeline-section">
      ...
      <div class="repay-timeline">${timelineHtml}</div>
    </div>
  `;
