...
| 👤 体验账号       | `admin` / `123456`（可在登录页注册新账户）                 |
| 📦 开源地址Github | https://github.com/xiangxiang088/ai-smart-audit |
| 📦 开源地址Gitee  | https://gitee.com/xiangxiang088/ai-smart-audit |
...
**测试资料**：仓库本地附有 5 组清标资料（`docs/bid-clearing-samples/`，含构造数据、未纳入版本库），覆盖清单级 / 小体量 / 总价级 / 装饰装修清单级 / 报价离散度五种场景；其中控制价与各投标人的投标总价均为真实公开数据，可用于功能验证。

| 组 | 工程类型 | 形态 | 规模 | 考点密度 |
|----|----------|------|------|----------|
| 第 1 组 | 市政道路 | 清单级 | 中 | 算术性错误、不平衡报价 |
| 第 2 组 | 市政道路 | 清单级（小体量） | 小 | 漏报、零报价，**解析最快** |
| 第 3 组 | 市政道路 | 总价级 | 中 | 报价离散度、费用构成 |
| **第 4 组** | **房建装饰装修** | **清单级** | **260 项节选** | **考点最密集（9 类）** |
| 第 5 组 | 房建装修改造 | 总价级 | 6 份文件 | 异常低价、费率不符、报价梯度 |

> 第 4 组是**首次覆盖房建装饰装修行业**的资料包：控制价与 3 家投标总价取自广西贺州富川朝东秀水游客中心装饰装修工程的真实公开数据，分项综合单价按真实清单结构测算生成。该组全为 Excel，秒级解析，是全量资料里**最快看到完整清标报告**的一组，建议作为首次自测首选。
...
> 📌 想**自己动手完整跑一遍**？见 **[主流程自测指引](docs/主流程自测指引.md)** —— 给出了每一步的按钮位置、上传清单、以及每条链路的**预期结果对照表**（含审计两项目的勾稽预期、清标第 4 组 9 类考点的"埋雷位置"）。
...
**审计主流程测试资料**：仓库本地附有 **2 个自洽的工程审计项目**（`docs/audit-samples/`，未纳入版本库），专为跑通「资料舱 → 核对程序 → 疑点台账 → 智能问答」而造。两份资料**内部同项目同编码**，所以清单↔结算勾稽能算出真实差异，而不是清一色的"无法确认"：

| 项目 | 工程类型 | 资料份数 | 勾稽预期（清单↔结算） | 疑点预期 |
|------|----------|----------|------------------------|----------|
| **项目A** | 房建装修改造 | 6 份 | **32 匹配 / 2 差异**（资料规范） | 低危为主 |
| **项目B** | 市政道路硬化 | 7 份 | **23 匹配 / 7 差异**（埋 7 处考点） | 三无签证、缺监理签章、连号发票、凑整合价 |

> 建议**先跑项目A 建立基线**（勾稽几乎全绿，用来看"正常长什么样"），再跑项目B 看各类异常能否被抓出来。项目A 的关键数字：分部分项 190,029.56、合计 208,473.16、审定造价 208,153.95、核减 5,846.05（2.73%）。
>
> 另：`docs/samples/06-projects-001/` 是**负样本**——它的清单来自安徽郎溪、结算来自广东东莞，两份资料**分属不同项目**，勾稽只会退化成"无法确认"。故意保留它，用来验证链路在资料不自洽时的兜底表现。
...
> 清标资料**不占用审计项目**，可独立使用；仓库本地另附 5 组清标测试资料（`docs/bid-clearing-samples/`，未纳入版本库）。
>
> **首次自测建议直接用第 4 组**（富川朝东秀水·装饰装修）：1 份控制价 + 3 家投标报价全是 Excel，秒级解析，260 项清单里埋了 9 类清标考点（算术性错误、不平衡报价、漏报、零报价、安全文明施工费偏低、工程量篡改、漏项、暂列金额下调、汇总与总价不一致），一份报告即可把各章节跑满。详见该目录下的 `README.md`。
...
│       ├── gen-clearing-group4.mjs      # 生成第 4 组清标资料（装饰装修·清单级）
│       ├── gen-clearing-group5.mjs      # 生成第 5 组清标资料（装修改造·总价级）
│       ├── gen-audit-packs.mjs          # 生成 2 个审计资料包（房建装修·资料规范 / 市政道路·埋考点）
│       ├── verify-audit-packs.mjs       # 审计资料包质检（清单列识别 / OCR 勾稽预期，--ocr 走真实 OCR）
│       ├── verify-clearing-pack.mjs     # 清标资料包质检（自洽性 / 考点命中）
...
├── docs/audit-samples/                  # 审计主流程测试资料（2 个项目：房建装修 / 市政道路）
├── docs/samples/                        # 负样本（06-projects-001：清单与结算分属不同项目）
└── docs/bid-clearing-samples/           # 清标测试资料包（5 组）
```
